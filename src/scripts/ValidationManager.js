...
class ValidationManager {
    ...
    async validate(selectedDays, options = {}) {
        ...
        if (!strategy.isApplicable(context)) {
            ...
            if (this.config.debug) {
                console.log('[ValidationManager] Strategy not applicable:', strategy.name);
            }
        ...
        if (this.config.debug) {
            console.log(`[ValidationManager] Running validation with strategy: ${strategy.name}`);
    ...
    getValidatorInfo() {
        return Array.from(this.strategies.values()).map(strategy => ({
            ...
            isActive: this.currentStrategy && this.currentStrategy.name === strategy.name
        }));
    }
    ...
    getSelectedDaysFromDOM() {
        ...
        if (this.config.debug) {
            console.log(`[ValidationManager] Found ${selectedDays.length} selected days from DOM`);
        }
