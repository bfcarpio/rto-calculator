...
class CalendarEventManager {
  private dragState: DragState = { isDragging: false, startCell: null, selectionIntent: null };
  ...
  private attachCalendarGridEvents(): void {
    ...
    grid.addEventListener('mousedown', this.handleGridMouseDown.bind(this) as EventListener);
    
    // Handle mouseover (for drag selection)
    grid.addEventListener('mouseover', this.handleGridMouseOver.bind(this) as EventListener);
    
    // Handle keyboard navigation
    grid.addEventListener('keydown', this.handleGridKeyDown.bind(this) as EventListener);
  ...
  private attachDocumentEvents(): void {
    ...
    document.addEventListener('mouseleave', this.handleDocumentMouseUp.bind(this));
  }
  ...
  private handleGridMouseDown(e: MouseEvent): void {
    ...
    const intendedSelection = (isSelected && selectionType === "out-of-office") ? null : "out-of-office";
  ...
  private navigateKeyboard(direction: string, currentCell: HTMLElement): void {
    const allCells = Array.from(document.querySelectorAll('.calendar-day:not(.empty)'));
  ...
  private applySelectionToCell(cell: HTMLElement, type: "out-of-office" | null): void {
    ...
    cell.dispatchEvent(new CustomEvent('rto-selection-changed', {
      bubbles: true,
  ...
  private attachClearButtonHandlers(): void {
    document.addEventListener('click', (e: Event) => {
      ...
      if (clearButton.id === 'clear-all-button-top' || clearButton.id === 'clear-all-button-bottom') {
  ...
  private clearAllSelections(): void {
    ...
    selectedCells.forEach(cell => this.applySelectionToCell(cell as HTMLElement, null));
  ...
  private clearMonthSelections(button: HTMLElement): void {
    ...
    const selectedCells = monthContainer.querySelectorAll('.calendar-day.selected');
  ...
  private updateTodayHighlight(): void {
    ...
    const allCells = document.querySelectorAll('.calendar-day[data-year][data-month][data-day]');
