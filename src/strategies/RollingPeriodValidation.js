...
class RollingPeriodValidation {
    ...
    validate(context) {
        ...
        const weeksByWFH = this._groupDaysByWeek(context.selectedDays, this.weekStart);
        ...
        const resultWindow = violatingWindows.length > 0
            ? violatingWindows[0]
        ...
        if (config.debug) {
            ...
            console.log(`[RollingPeriodValidation] Total windows: ${windowResults.length}`);
            console.log(`[RollingPeriodValidation] Violating windows: ${violatingWindows.length}`);
        }
    ...
    getWeekCompliance(weekStart, context) {
        ...
        const weeksByWFH = context.weeksByWFH || this._groupDaysByWeek(
        ...
        if (config.debug) {
            console.log(`[RollingPeriodValidation] Week ${weekKey}: ${officeDays}/${config.totalWeekdaysPerWeek} office days (${percentage.toFixed(0)}%)`);
        }
    ...
    getWindowCompliance(windowStart, windowSize, context) {
        ...
        for (let weekIndex = windowStart; weekIndex < windowStart + windowSize; weekIndex++) {
        ...
        const compliancePercentage = totalWeekdays > 0
            ? (totalOfficeDays / totalWeekdays) * 100
            : 100;
        const isCompliant = compliancePercentage >= config.thresholdPercentage * 100;

        if (config.debug) {
            console.log(`[RollingPeriodValidation] Window ${windowStart}-${windowStart + windowSize - 1}: ${averageOfficeDaysPerWeek.toFixed(1)} avg office days (${compliancePercentage.toFixed(0)}%)`);
        }
    ...
    _getTotalWeeks(context) {
        if (context.calendarStartDate && context.calendarEndDate) {
            const diffTime = context.calendarEndDate.getTime() - context.calendarStartDate.getTime();
            const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
