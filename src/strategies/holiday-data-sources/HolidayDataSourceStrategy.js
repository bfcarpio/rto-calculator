...
class HolidayDataSourceStrategy {
  ...
  async getHolidaysForYear(year, countryCode) {
    ...
    if (this.config.debug) {
      console.log(`[HolidayDataSource] Fetching holidays for ${countryCode} ${year}`);
    }
  ...
  async _fetchHolidaysForYear(year, countryCode) {
    throw new Error(
      `_fetchHolidaysForYear must be implemented by subclass`,
    );
  }
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = HolidayDataSourceStrategy;
} else {
  window.HolidayDataSourceStrategy = HolidayDataSourceStrategy;
}
