...
export class CalendarEventHandlers {
	...
	private attachCalendarGridEvents(): void {
		...
		const mousedownHandler = (e: Event) =>
			this.handleGridMouseDown(e as MouseEvent);
		grid.addEventListener("mousedown", mousedownHandler);
		this.cleanupFunctions.push(() =>
			grid.removeEventListener("mousedown", mousedownHandler),
		);

		// Handle mouseover (drag over cells)
		const mouseoverHandler = (e: Event) =>
			this.handleGridMouseOver(e as MouseEvent);
		...
		const keydownHandler = (e: Event) =>
			this.handleGridKeyDown(e as KeyboardEvent);
		grid.addEventListener("keydown", keydownHandler);
		this.cleanupFunctions.push(() =>
			grid.removeEventListener("keydown", keydownHandler),
		);
	}
	...
	private handleGridMouseDown(e: MouseEvent): void {
		...
		const intendedSelection: DateState | null = currentState
			? null
			: defaultState;
	...
	private handleGridMouseOver(e: MouseEvent): void {
		...
		dragStateStore.set({
			isDragging: true,
			startPoint:
				(this.dragState.startCell?.dataset.date as DateString | null) || null,
			currentPoint: dateStr,
	...
	private navigateKeyboard(direction: string, currentCell: HTMLElement): void {
		const allCells = Array.from(
			this.container.querySelectorAll(
				".datepainter__day[data-date]:not(.datepainter__day--empty)",
			),
		);
	...
	clearAllSelections(): void {
		const selectedCells = this.container.querySelectorAll(
			".datepainter__day[data-state]",
		);
	...
	clearMonthSelections(monthId: string): void {
		const monthContainer = this.container.querySelector(
			`[data-month="${monthId}"]`,
		);

		if (!monthContainer) {
			console.warn(
				`[CalendarEventHandlers] Month container not found: ${monthId}`,
			);
			return;
		}
